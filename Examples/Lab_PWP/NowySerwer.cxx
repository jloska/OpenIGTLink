...
int ReceivePoint(igtl::Socket * socket, igtl::MessageHeader * header)
{
	...
	igtl::PointMessage::Pointer pointMessageSend;
	pointMessageSend = igtl::PointMessage::New();
	pointMessageSend->SetDeviceName("PointSender");
	...
	if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
	{
		...
		for (int i = 0; i < nElements; i++)
		{
			...
			pointMessageSend->AddPointElement(negpoint);
		}
		igtl::Sleep(5000);
		pointMessageSend->Pack();
		socket->Send(pointMessageSend->GetPackPointer(), pointMessageSend->GetPackSize());
	}
